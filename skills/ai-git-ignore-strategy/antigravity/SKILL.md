...
description: 建立並套用針對各式 AI 代理工具 (Antigravity, Claude Code, Cursor, Codex, Gemini 等) 的 .gitignore 最佳實務，防止專案庫被 AI 對話紀錄與暫存撐爆；同時正確保留設計藍圖、部署參考、備份資料檔與團隊 AI 指令檔。也負責跨平台（Linux VM ↔ Windows SSHFS）行尾正規化與 .gitattributes 設定。當使用者要求「檢核 gitignore」「整理 repo」「commit 前審查」「清理 AI 追蹤紀錄」「行尾 CRLF 問題」時觸發。
---

# AI 工作區 Git 管控最佳實務 (AI Git Ignore Strategy)

當使用者用各類代理型 AI 工具（Antigravity、Claude Code、Cursor、Gemini、Codex⋯⋯）進行本地開發時，AI 系統會在專案根目錄產生隱藏的本地狀態與對話紀錄資料夾（如 `.gemini/`, `.claude/`, `.agent/` 等）。
...
- **AI 對話紀錄與快取**：`.gemini/`, `.agent/`, `.claude/`, `.codex/`, `.cursor/`, `.github/prompts/` 底下非 skills 的 session logs、快取、索引檔。
  - ⚠️ 注意：`.gemini/skills/` 是客製技能可能要保留（見「需確認」類）；`.gemini/` 底下的其他資料夾（`brain/`, `scratch/` 等）才是純暫存。
...
- **AI 技能庫 (Skills)**：`.gemini/skills/`, `.claude/skills/`, `~/.gemini/antigravity/skills/` 等目錄。
...
> 例如：`.gemini/` 會完全忽略整個資料夾，此時 `!.gemini/skills/` **無效**。
> 必須改為 `.gemini/*` 搭配 `!.gemini/skills/` 才能正確放行。
