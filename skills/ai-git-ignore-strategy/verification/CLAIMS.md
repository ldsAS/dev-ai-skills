...
| C-04 | `.claude/commands/`、`.claude/agents/` | 團隊共用，應提交　📌 **2026-09-11 佐證（Issue #12 追加訊號）**：skills 頁新增「command name source」表格，其中一列為「File in a subdirectory of `.claude/commands/` — Subdirectory path relative to `commands/` with each `/` replaced by `:`, then the file name without extension — `.claude/commands/frontend/component.md` → `/frontend:component`」。→ **子目錄內的 command 檔屬既有機制**，CHANGELOG **1.0.45** 即有同一個例子逐字出現（原文用 **Restored**，故機制更早於此）。範本實測：`.claude/commands/deploy.md` 與 `.claude/commands/frontend/component.md` **皆放行**，`!.claude/commands/` 的白名單涵蓋得到巢狀檔案 → **無需異動**。⚠️ 本次告警的真正成因與本條主張無關，而是**逐來源比對**：該 token 早已存在於 `claude-code/changelog` 的 baseline，官方只是把同一個例子複製進 skills 頁（見 C-101） | 官方文件 | 2026-07-29 | — | 已驗證 |
...
| C-104 | `.claude/hooks/` | 官方範例將專案 hook 腳本放於此處，由團隊 `.claude/settings.json` 引用。範本預設放行，避免共享設定缺少腳本；**放行是本 skill 的追蹤政策，不是官方保證每個腳本都應公開**。提交前逐檔審查；只供私人設定引用的腳本另列精準排除。既有 `.env`、`*.log` 規則仍生效 **執行界線（09-24 文件複閱）**：腳本只有被已啟用的 hook 設定引用才會執行，放進本目錄不等於自動執行；VS Code Local harness 的 Claude 格式相容載入另需 `chat.useClaudeHooks`，不代表所有 VS Code harness 皆須使用 Local。見 C-77 及 [VS Code hooks](https://code.visualstudio.com/docs/copilot/customization/hooks)。 | 官方 hooks 文件＋五版 Git 邊界實測 | 2026-09-21 | 文件快照；Git 實測 | 已驗證 |
| C-105 | `.claude/agent-memory/`、`.claude/agent-memory-local/` | **M1 與 C-102 已於 09-23 經 PR #18 合併（403a3f5）**：`claude-code/claude-directory` 含兩個目錄的精準 token。此前父目錄導致的假涵蓋與來源缺口已分別修正；歷史分支驗收見 M1／M0 紀錄。前者是 project-scope subagent 記憶，官方定位為團隊共享，範本放行但仍審查內容。`memory: local` 使用後者，範本以 `**/.claude/agent-memory-local/` 排除根目錄與子目錄中的同名目錄；子目錄案例驗證規則邊界，不宣稱工具必然自動在子目錄產生它。主 session 的家目錄 auto memory 是另一機制 | 官方 claude-directory 文件＋五版 Git 邊界實測 | 2026-09-21 | 文件快照；未執行 Claude runtime | 已驗證 |
...
| C-14 | `.agents/plugins/marketplace.json` | 專案層外掛市集，官方定位為團隊共用，應提交　📌 **2026-09-10 佐證（Issue #12 同日追加訊號）**：Codex `config-reference` 新增 `marketplaces.*` 設定鍵，明載「`marketplaces.<name>.source` — Git repository location or local marketplace root directory. Use an absolute path for a local source; **the directory contains `.agents/plugins/marketplace.json`**」，並在 `sparse_paths` 補上「Include the marketplace catalog **and any local plugin directories it references**」。→ 本條**未被推翻，且被加強**：manifest 路徑不變，外掛本體確實另存於它引用的 local plugin directories。範本三段式規則與官方文字一致，**無需異動**。另注意 local 市集的 `source` 要求絕對路徑，市集根目錄可完全位於專案之外（同 C-91 情境，不產生專案層規則需求） | 官方文件 | 2026-07-29 | — | 已驗證 |
| C-15 | `.agents/plugins/*` | 官方寫明專案層只放 `.agents/plugins/marketplace.json`，**外掛本體另存於 repo-local 目錄（如 `./plugins/`）**，不在 `.agents/plugins/` 底下。現行「放行 manifest、擋其餘」的三段式規則正確　📌 **2026-09-10 佐證（Issue #12 同日追加訊號）**：Codex `config-reference` 新增 `marketplaces.*` 設定鍵，明載「`marketplaces.<name>.source` — Git repository location or local marketplace root directory. Use an absolute path for a local source; **the directory contains `.agents/plugins/marketplace.json`**」，並在 `sparse_paths` 補上「Include the marketplace catalog **and any local plugin directories it references**」。→ 本條**未被推翻，且被加強**：manifest 路徑不變，外掛本體確實另存於它引用的 local plugin directories。範本三段式規則與官方文字一致，**無需異動**。另注意 local 市集的 `source` 要求絕對路徑，市集根目錄可完全位於專案之外（同 C-91 情境，不產生專案層規則需求） | 官方 changelog | 2026-07-30 | — | 已驗證 |
...
| C-24 | `~/.gemini/tmp/`（session 實際位置） | Session 存於 **家目錄** `~/.gemini/tmp/<project_hash>/chats/`，不寫入專案。專案內 `.gemini/` 以 `settings.json`、`skills/` 等共享內容為主，無需額外 cache 排除規則 **人工複驗盲區**：本條包含正向儲存位置事實；即使不寫入專案，仍須核對上游搬移，不能視為純反向主張而移出監控需求。 | 官方文件 | 2026-07-30 | — | 已驗證 |
...
| C-77 | Copilot 跨工具讀取 `.claude/`、`.agents/` | 官方明載 Copilot 會讀 `.claude/skills/`、`.agents/skills/`、`~/.claude/skills/`、`~/.agents/skills/`，以及 Claude 格式的 `.claude/settings.json`、`.claude/settings.local.json`（hooks）。**`.claude/` 並非 Claude Code 專屬** —— 繼 C-60 的 `.agents/` 之後，`.claude/` 同樣是跨工具目錄。`.claude/skills/*` 的 scoped allowlist 因此同時影響 Copilot **2026-09-24 hooks 範圍補註**：透過 VS Code **Local harness** 載入上述 Claude 格式 hooks 設定時，需 `chat.useClaudeHooks`（預設關閉），並受 Local 執行開關與 workspace trust 等條件控制；Agent Host 的各 harness 另有實作，事件／payload／matcher 不保證相容。本限制只修飾 hooks 相容功能，不能套到前述 skills 讀取。來源：[VS Code hooks](https://code.visualstudio.com/docs/copilot/customization/hooks)，文件證據，未執行 runtime。 | 官方文件 agent-skills／hooks 頁 | 2026-08-03 | — | 已驗證 |
...
| C-41 | `.agents/AGENTS.md` | **以下為 agy 1.0.13／當時受檢環境的歷史觀察，不是現行版本的無條件結論。2026-09-24 現行文件與政策見 C-115。** ~~專案級設定，應提交~~ → **不存在**；Antigravity 只讀根目錄的 `AGENTS.md`。**2026-08-04 白名單規則已自五版移除**：agy 1.0.13 二進位中 `{workspace}/.agents/` 的路徑模板完整清單只有 `skills`、`agents`、`ORIGINAL_REQUEST.md` 三條，本條不在內；本機五個實際專案的 `.agents/` 亦 0 次出現。原先保留為「防禦性白名單」，但為不存在的路徑留規則會讓讀者誤以為該路徑有效 —— 同 `.codex/rollout.jsonl` 的處置 | 實機＋維護者複驗 | 2026-08-04 | 1.0.13 | 被取代→C-115 |
| C-42 | `.agents/settings.json` | **以下為 agy 1.0.13／當時受檢環境的歷史觀察，不是現行版本的無條件結論。2026-09-24 現行文件與政策見 C-116。** ~~專案級設定，應提交~~ → **不存在**；專案設定實存於 `~/.gemini/config/projects/<uuid>.json`。**2026-08-04 白名單規則已自五版移除**：agy 1.0.13 二進位中 `{workspace}/.agents/` 的路徑模板完整清單只有 `skills`、`agents`、`ORIGINAL_REQUEST.md` 三條，本條不在內；本機五個實際專案的 `.agents/` 亦 0 次出現。原先保留為「防禦性白名單」，但為不存在的路徑留規則會讓讀者誤以為該路徑有效 —— 同 `.codex/rollout.jsonl` 的處置 | 實機＋官方 changelog（雙重佐證） | 2026-08-04 | 1.0.13 | 被取代→C-116 |
| C-43 | `.agents/settings.local.json` | ~~個人本機設定~~ → **不存在**，係誤類比 Claude Code；規則已移除 **否定結論僅限 agy 1.0.13 與當時受檢環境；本輪未取得新反證，不推定所有後續版本都不存在。** | 實機＋維護者複驗 | 2026-07-29 | 1.0.13 | 已驗證 |
...
| C-50 | `AGENTS.md`（根目錄） | Antigravity 已支援讀取，與 `GEMINI.md` 並列 2026-09-24 文件另外列出目錄層與 `.agents/` 內規則（C-115），不推翻本條根目錄的正向事實，也不表示只讀根目錄。 | 官方 changelog＋實機 | 2026-07-29 | 1.0.13 | 已驗證 |
...
| C-55 | `.agents/ORIGINAL_REQUEST.md` | agent 會把使用者訊息**逐字**附加至此檔（含 UTC 時間戳），亦有 `.agents/<agent_folder>/ORIGINAL_REQUEST.md` 變體。**屬敏感內容，必須排除** 本輪保留 1.0.13 二進位證據，未重跑 runtime。monorepo 子目錄 `.agents/` 的同名檔防禦政策另列 C-117，不把規則驗證誤當實際寫入證據。 | 二進位字串分析 | 2026-07-30 | 1.0.13 | 已驗證 |
...
| C-98 | token 的前導斜線正規化（**非路徑主張**，故不列入監控涵蓋率） | **`_norm_path()` 原本只做 `rstrip("/")`，前導斜線一律留著 → 這類 token 永遠對不到任何主張。** 官方文件常以佔位符前綴書寫路徑，佔位符被 `TOKEN_RE` 切掉後只剩分隔用的 `/`：skills 頁的 `<subdir>/.claude/skills/<skill-name>/SKILL.md` 抽出來就是 `/.claude/skills/`。Issue #12 因此把一條**早已登記的 C-06 路徑**列進「帳本查無對應主張的新路徑」，讓維護者重複分類。實測 baseline 共 **13 個**前導斜線 token（跨 5 個來源），**全部** match 0 條；去掉前導斜線後**每一個**都對上既有主張（C-06／C-03／C-62／C-13／C-22／C-44／C-11／C-17）。→ 改為 `strip("/")`。⚠️ **只動主張比對，不動異動偵測**：`token_comparison_key()` 維持原樣，baseline 仍以原文拼法比對，因此「文件新增一個帶佔位符前綴的寫法」照樣會告警 —— 修的是**歸屬**，不是靈敏度。副作用經實測為純增益：涵蓋率不變（50／盲區 2／共 52），僅 C-44 多認得一個來源（antigravity/changelog） | baseline 全量重跑＋回歸測試（還原 `rstrip` 則 3 項轉紅） | 2026-09-10 | claude-code 2.1.266 | 已驗證 |
| C-99 | 單段 token 不向下扇出（**非路徑主張**，故不列入監控涵蓋率） | **單段目錄 token 經雙向前綴比對會命中它底下的每一條主張。** Issue #12 中官方 skills 頁只是改寫行文、每一處 `.claude` 都帶了更長後綴、不再出現裸 `.claude/`，就讓 C-02、C-03、C-04、C-05、C-06、C-09、C-53、C-62、C-83、C-92 **十條**同時被標成「原依據可能已不成立」。實際上 `.claude/` 仍存在於另外三個 claude-code 來源的 baseline（changelog、memory、settings）—— 它只是離開了 skills 頁，沒有離開官方文件。`_claim_paths()` 早就把單段目錄逐出**帳本側**比對範圍（理由：範圍過寬會失去指向性），但 **token 側沒有對稱規則**。→ `claims_for_token()` 對單段 token 只做等值比對，報告並加註「單段目錄語彙訊號：僅比對同名主張，不代表其下各主張失效」。⚠️ **刻意接受的靈敏度損失（比照 C-96 的取捨聲明）**：若官方日後真的整個移除某工具目錄，報告只會標到「以該目錄本身為主張」者（如 `.claude` 的 C-62），不再自動列出其下全部主張；維護者需自行往下追。判斷依據是**單段目錄的存在與否，本來就不是其下各路徑是否成立的證據** —— 但這是損失，不是沒有損失。等值比對仍保留單段**檔名**主張（`.geminiignore`、`.mcp.json`）的命中 | 多來源 baseline 交叉比對＋回歸測試（移除守衛則 3 項轉紅） | 2026-09-10 | claude-code 2.1.266 | 已驗證 |
| C-100 | 單段**主張路徑**不被深層 token 往上命中（**非路徑主張**，故不列入監控涵蓋率） | **C-99 只收斂了 token 側，帳本側是同一個問題的鏡像。** C-62 的主張路徑是單段的 `.claude`，於是**任何** `.claude/**` 多段 token 都會前綴命中它 —— 實測 baseline 156 個 token 中有 **26 個**命中 C-62，其中 **24 個純屬噪音**（`.claude/settings.json`、`.claude/skills/`、`.claude/commands/frontend/component.md`⋯⋯）。`.claude/skills/` 的變動不構成「`.claude` 這個路徑本身可否為 symlink」的證據。→ 前綴比對改為要求**雙方都是多段路徑**，單段的一方只做等值比對。修正後命中 C-62 的只剩 `.claude`、`.claude/` 兩個。⚠️ **刻意接受的損失**：若上游把整個 `.claude/` 改名，C-62 仍會由等值比對命中（裸 token 會消失），但「`.claude` 底下某個檔被改名」不再牽動 C-62 —— 那本來就不是它的證據。單段**檔名**主張（`.geminiignore`、`.mcp.json`、`AGENTS.md` 等 9 條）不受影響，因為它們本來就不存在「更深的 token」 | baseline 全量實測＋回歸測試（移除守衛則 1 項轉紅） | 2026-09-11 | claude-code 2.1.268 | 已驗證 |
| C-101 | 跨來源存在性必須在報告中說明（**非路徑主張**，故不列入監控涵蓋率） | **比對是逐來源進行的，所以官方把同一段說明換頁會被讀成機制變動。** 兩個實例：① Issue #12 的 `.claude/` 報「消失」，但它**仍存在於** `claude-code/changelog`、`claude-code/memory`、`claude-code/settings` 三個來源；② 2026-09-11 的 `.claude/commands/frontend/component.md` 報「新增」，但它**早已存在於** `claude-code/changelog` 的 baseline（即 CHANGELOG 1.0.45 的同一個例子）—— 兩次都讓維護者重新分類一條已知路徑。→ 報告在消失項加註「此路徑目前仍出現在 …」（2026-09-23 修正：只查 `verified_sources`，即本輪成功抓取且通過空集合檢查者；先前查 `new_sources` 會把失敗時沿用的舊 baseline 誤說成目前仍存在，此 P2 已修正），在新增項加註「此路徑在本次之前已存在於 …」（查 `baseline_sources`，故**兩個來源同時新增**仍會被當成真正的新候選）。⚠️ **刻意只註記、不抑制**：偵測與 baseline 寫回行為完全不變，因為「某頁開始／停止提到某路徑」本身仍可能有意義（例如官方正式棄用某位置）；改的是**報告怎麼說**，不是要不要報 | Issue #12 的兩則實例＋baseline 交叉比對＋回歸測試（移除註記則 2 項轉紅，另有一項保證真正的新 token 不被加註） | 2026-09-11 | claude-code 2.1.268 | 已驗證 |
| C-102 | 涵蓋率報表與告警歸屬必須同規則（**非路徑主張**，故不列入監控涵蓋率） | **`report_coverage()` 自己內嵌了一份比對邏輯，與 `claims_for_token()` 分家 —— 於是「有涵蓋」長期被高估。** 寬鬆規則會把「不可能見證該路徑變動」的來源也算進去：`.agents/ORIGINAL_REQUEST.md`(C-55) 只因為某來源有一個裸 `.agents/` token 就被算成有涵蓋，但上游改掉那個檔名時裸 `.agents/` 不會變，監控不會響。→ 改為直接呼叫 `claims_for_token()`。**結果：盲區由 2 條修正為 7 條（涵蓋 51 → 46／共 53）**，新浮現的 5 條為 **C-83** `.claude/launch.json`、**C-41** `.agents/AGENTS.md`、**C-42** `.agents/settings.json`、**C-43** `.agents/settings.local.json`、**C-55** `.agents/ORIGINAL_REQUEST.md`。📌 **監控並沒有變差，是報表變誠實了**：這 5 條的依據本來就不是受監控的官方頁 —— C-41／C-42 主張的是「此檔**不存在**」（上游無物可watch，盲區在此屬恆真而非缺口），C-83 與 C-55 皆由**實地觀察**取證（見各條的「發現經過」），C-43 的規則已移除。→ 真正的行動項只有「這 5 條的複驗不能依賴排程」，已記入〈監控來源缺口〉 | 統一前後全量比對（24 條主張的涵蓋來源清單改變）＋回歸測試（還原內嵌比對則 1 項轉紅） | 2026-09-11 | claude-code 2.1.268 | 已驗證 |
...
| 2026-09-10 | 排程偵測 → Claude 分類與修正 | Issue #12 的兩個候選 token | [分類紀錄](./rounds/2026-09-10-claude.candidate-triage.md) | **兩個訊號皆判定為語彙假陽性，10 條受影響主張全數未被推翻** —— `.claude/` 只是離開 skills 頁（仍在另外三個來源），`/.claude/skills/` 是 `<subdir>/` 佔位符殘留、正確歸屬 C-06，且 nested skills 機制早在 CHANGELOG 2.1.178 就存在，本次變的是文件把它寫進表格。分類過程另外查出一條**範本實質缺口**：monorepo 子目錄的 `.claude/settings.local.json` 未被根目錄錨定規則涵蓋（→ **C-97**，五版皆改）。兩個監控缺陷同步修掉（→ **C-98** 前導斜線、**C-99** 單段扇出），回歸測試 26 → 33 項、範本邊界案例 47 → 51 個，兩者皆以「還原修正即轉紅」驗證過斷言有效。同日 06:01 追加的第三個訊號（Codex `config-reference` 的 `.agents/plugins/marketplace.json`）**判定為佐證非異動** —— C-14、C-15 均被官方新增的 `marketplaces.*` 設定鍵加強，範本無需異動 |
| 2026-09-11 | 排程偵測 → Claude 分類與修正 | Issue #12 的 2026-09-11 追加訊號 | [分類紀錄](./rounds/2026-09-11-claude.cross-source-triage.md) | **判定為文件範例，C-04 未被推翻、範本零異動** —— skills 頁新增命名表，把 CHANGELOG 1.0.45 的既有例子 `.claude/commands/frontend/component.md` 複製進來；該 token 早已在 `claude-code/changelog` 的 baseline 裡。分類過程循 C-62 的噪音往下查，發現三件事：**C-100** 帳本側單段路徑同樣會誤命中（24/26 為噪音）、**C-101** 逐來源比對會把換頁讀成異動（Issue #12 的兩個訊號都是這一類）、**C-102** 涵蓋率報表與告警歸屬用了兩套規則，**盲區由 2 條修正為 7 條**。回歸測試 33 → 41 項，四種還原情境皆確認轉紅 |
