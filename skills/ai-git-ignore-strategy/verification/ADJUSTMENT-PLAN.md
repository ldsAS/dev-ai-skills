...
5. ~~P2-1 重跑安裝器~~ → 2026-08-04 已執行。六份全域副本核對相符後，
   刪除兩條舊版相容路徑的複本，**收斂為四份**（見 README「安裝邏輯」）。

## 仍然開著的兩件事

| 項目 | 為什麼還沒結 | 誰能結 |
| :--- | :--- | :--- |
| **C-54** `.agents/agents/<name>/agent.json` 的實體內容 | 路徑已由二進位模板證實，但**沒有人看過真實檔案**。這條是白名單（放行提交），依本專案的稽核原則屬高風險側 | Antigravity（建一個 workspace agent 再看檔案） |
| **移除舊路徑後各工具是否仍載入得到** | `~/.codex/skills`、`~/.copilot/skills` 的複本已刪，依據是官方文件說兩者都讀 `~/.agents/skills`（C-12、C-77）。若該文件說法與實機不符，技能會**無聲消失** | Codex、Copilot 各回報一次載入來源 |

> 後者正是本專案 README 強調的「**路徑消失比新增更嚴重**」—— 我們剛主動移除了路徑，
> 而移除的依據是文件而非實機。這是典型的 (c) 類漂移風險，只有到實機問才問得出來。
