...
每個 skill 都必須提供 **三個版本** 的 `SKILL.md`：

```text
skills/<skill-name>/
├── README.md              # [必要] skill 簡介 + 三版本差異說明
├── antigravity/
...
> 💡 **三個版本的 `description` 可以幾乎一樣**，但 body 內容會因工具差異而不同。
...
- [ ] 三個 `SKILL.md` 都有正確的 frontmatter（`name` + `description`）
- [ ] 三個 `name` 欄位與資料夾名稱完全一致
...
- [ ] 撰寫 `skills/<skill-name>/README.md` 說明三版本差異與使用時機
...
```bash
# 1. 建立資料夾骨架
mkdir -p skills/my-new-skill/{antigravity,claude,generic}

# 2. 撰寫三個 SKILL.md（可以先寫 generic 版再往兩邊 port）
