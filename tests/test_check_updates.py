...
class MonitorRunMixin:
    ...
    def _run(
        ...
        *,
        ...
        # extra_sources 的值可為頁面文字或例外；可重現部分來源失敗，
        # 同時讓其他來源正常回傳的情境（C-101）。
        ...
        def fake_fetch(url):
            ...
            if isinstance(texts[url], Exception):
                raise texts[url]
            return texts[url]
...
class CrossSourceNoteTests(MonitorRunMixin, unittest.TestCase):
    ...
    def _check_unverified_source(self, body, failure_reason):
        token = ".claude/hooks/"
        baseline = self._baseline_with_other_source(
            [token, ".claude/settings.json"],
            [token, ".claude/rules/", ".claude/skills/"],
        )
        for dry_run in (False, True):
            with self.subTest(dry_run=dry_run):
                report, before, after = self._run(
                    baseline=baseline,
                    source_text=".claude/settings.json",
                    extra_sources={self.other_key: body},
                    dry_run=dry_run,
                )
                self.assertIn(f"**消失**：`{token}`", report)
                self.assertIn("[SIGNAL: UPDATE_DETECTED]", report)
                self.assertIn("[SIGNAL: CHECK_FAILED]", report)
                self.assertIn(failure_reason, report)
                self.assertNotIn("此路徑目前仍出現在", report)
                # 說明必須反映未驗證，但失敗來源的 baseline 仍應保留。
                self.assertEqual(baseline["sources"][self.other_key],
                                 json.loads(after)["sources"][self.other_key])
                if dry_run:
                    self.assertEqual(before, after)
                else:
                    self.assertEqual([".claude/settings.json"],
                                     json.loads(after)["sources"][self.source_key]["tokens"])

    def test_failed_other_source_cannot_confirm_current_presence(self):
        self._check_unverified_source(RuntimeError("fixture outage"), "fixture outage")

    def test_suspicious_empty_source_cannot_confirm_current_presence(self):
        body = '<!doctype html><meta http-equiv="refresh" content="0;url=/new-page">'
        self._check_unverified_source(body, "本次保留舊基準")

    def test_mixed_sources_only_name_successfully_checked_evidence(self):
        baseline = self._baseline_with_other_source(
            [".claude/hooks/", ".claude/settings.json"],
            [".claude/hooks/", ".claude/rules/", ".claude/skills/"],
        )
        healthy_key = "claude-code/hooks"
        baseline["sources"][healthy_key] = {"tokens": [".claude/hooks/"]}
        report, _, after = self._run(
            baseline=baseline,
            source_text=".claude/settings.json",
            extra_sources={self.other_key: RuntimeError("fixture outage"),
                           healthy_key: ".claude/hooks/"},
        )
        notes = [line for line in report.splitlines() if "此路徑目前仍出現在" in line]
        self.assertEqual(1, len(notes))
        self.assertIn(f"`{healthy_key}`", notes[0])
        self.assertNotIn(f"`{self.other_key}`", notes[0])
        self.assertIn("[SIGNAL: CHECK_FAILED]", report)
        self.assertIn("[SIGNAL: UPDATE_DETECTED]", report)
        self.assertEqual(baseline["sources"][self.other_key],
                         json.loads(after)["sources"][self.other_key])
